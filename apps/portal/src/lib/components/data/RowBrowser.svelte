<script lang="ts">
  ...
  const mayEdit = $derived(
    table.level === "edit" || table.level === "full",
  );
  ...
  /*
   * reverseOrder reverses the actual database query ordering.
   *
   * This is intentionally done before pagination so "Newest first"
   * retrieves the newest rows in the whole table, rather than merely
   * reversing the current page.
   */
  const effectiveDesc = $derived(
    reverseOrder ? !desc : desc,
  );
  ...
  $effect(() => {
    /*
     * Reading request here makes changes to reverseOrder, sorting,
     * pagination, search, or stamp trigger a new server request.
     */
