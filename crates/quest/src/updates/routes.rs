...
mod ranges {
    ...
    #[test]
    fn reads_the_forms_the_plugin_sends() {
        assert_eq!(span("bytes=0-", 100), Some((0, 99)));
        assert_eq!(span("bytes=40-", 100), Some((40, 99)));
        assert_eq!(span("bytes=0-9", 100), Some((0, 9)));
        assert_eq!(span("bytes=-10", 100), Some((90, 99)));
    }

    #[test]
    fn clamps_an_overlong_end_instead_of_refusing() {
        assert_eq!(span("bytes=90-500", 100), Some((90, 99)));
    }

    #[test]
    fn refuses_what_it_cannot_serve() {
        assert_eq!(span("bytes=100-", 100), None);
        assert_eq!(span("bytes=50-40", 100), None);
        assert_eq!(span("bytes=0-1,5-6", 100), None);
        assert_eq!(span("items=0-", 100), None);
        assert_eq!(span("bytes=-", 100), None);
        assert_eq!(span("bytes=0-", 0), None);
    }
}
...
mod origins {
    ...
    fn headers(pairs: &[(&'static str, &str)]) -> HeaderMap {
        ...
        for (name, value) in pairs {
            out.insert(*name, HeaderValue::from_str(value).unwrap());
        }
        out
    }

    #[test]
    fn trusts_the_proxy_scheme_when_present() {
        let sent = headers(&[("host", "cmu.quest"), ("x-forwarded-proto", "https")]);
        assert_eq!(origin(&sent).as_deref(), Some("https://cmu.quest"));
    }

    #[test]
    fn takes_the_first_hop_of_a_chained_proto() {
        let sent = headers(&[("host", "cmu.quest"), ("x-forwarded-proto", "https, http")]);
        assert_eq!(origin(&sent).as_deref(), Some("https://cmu.quest"));
    }

    #[test]
    fn assumes_plaintext_only_on_a_local_host() {
        let sent = headers(&[("host", "192.168.1.5:8080")]);
        assert_eq!(origin(&sent).as_deref(), Some("http://192.168.1.5:8080"));
    }

    #[test]
    fn assumes_tls_for_anything_public() {
        let sent = headers(&[("host", "cmu.quest")]);
        assert_eq!(origin(&sent).as_deref(), Some("https://cmu.quest"));
    }

    #[test]
    fn refuses_without_a_host() {
        assert_eq!(origin(&HeaderMap::new()), None);
        let mut blank = HeaderMap::new();
        blank.insert(header::HOST, HeaderValue::from_static(""));
        assert_eq!(origin(&blank), None);
    }
}
